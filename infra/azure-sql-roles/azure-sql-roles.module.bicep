...
param azure_sql_outputs_name string

param azure_sql_outputs_sqlserveradminname string
...
resource azure_sql 'Microsoft.Sql/servers@2021-11-01' existing = {
  name: azure_sql_outputs_name
}

resource sqlServerAdmin 'Microsoft.ManagedIdentity/userAssignedIdentities@2023-01-31' existing = {
  name: azure_sql_outputs_sqlserveradminname
}
...
resource script_azure_sql_user_db 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  name: take('script-${uniqueString('azure_sql', principalName, 'user-db', resourceGroup().id)}', 24)
  location: location
  identity: {
    type: 'UserAssigned'
  ...
  properties: {
    ...
    environmentVariables: [
      ...
      {
        name: 'DBSERVER'
        value: azure_sql.properties.fullyQualifiedDomainName
      }
      {
        name: 'PRINCIPALTYPE'
        value: principalType
...
resource script_azure_sql_policy_db 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  name: take('script-${uniqueString('azure_sql', principalName, 'policy-db', resourceGroup().id)}', 24)
  location: location
  identity: {
    type: 'UserAssigned'
    userAssignedIdentities: {
...
resource script_azure_sql_order_db 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  name: take('script-${uniqueString('azure_sql', principalName, 'order-db', resourceGroup().id)}', 24)
  location: location
  identity: {
    type: 'UserAssigned'
    userAssignedIdentities: {
